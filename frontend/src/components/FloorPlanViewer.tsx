...
export default function FloorPlanViewer({ result }: Props) {
  ...
  // Reset all per-floor state whenever a new model is generated
  useEffect(() => {
  ...
  // Fetch SVG for upper floors on demand; check r.ok before using the body
  useEffect(() => {
    ...
    fetch(floorplanUrl(result.model_id, activeFloor))
      ...
      .catch(() => {
        setFloorStatus(prev => ({ ...prev, [activeFloor]: 'error' }))
      })
  ...
  // Apply layer visibility to the inline SVG after any relevant state change
  useEffect(() => {
  ...
  // Compass drag
  const startDrag = (e: React.MouseEvent) => { e.preventDefault(); setDragging(true) }
      ...
      <div className="relative rounded-xl border border-slate-100 bg-slate-50">
          ...
          {status === 'error' ? (
            <div className="flex flex-col items-center justify-center p-10 gap-3">
              <span className="text-sm text-red-500 font-medium">
                Failed to load Floor {activeFloor + 1}
              </span>
              <button
                onClick={() => retryFloor(activeFloor)}
        ...
        <div className="absolute top-3 right-3 flex flex-col items-center gap-1 select-none z-10">
          <svg
            ...
            title="Drag to set north direction"
          >
            ...
            <g transform={`rotate(${northAngle})`}>
              ...
              <text x="0" y="-26" textAnchor="middle" dominantBaseline="middle"
                fontSize="9" fontWeight="700" fill="#ef4444"
