        ...
        return (
          <div key={cat}>
            <button onClick={() => toggle(cat)}
              className={`w-full flex items-center justify-between px-3 py-2 text-sm font-medium ${CAT_COLOR[cat] || 'bg-gray-50 text-gray-600'}`}>
              <span>{cat}</span>
                ...
                <tbody>
                  {catItems.map((item, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="py-1.5 px-3 text-gray-600">{item.item}</td>
                      <td className="py-1.5 px-3 text-right font-mono text-gray-800 font-medium">
                        {item.quantity.toLocaleString(undefined, { maximumFractionDigits: 1 })}
                      </td>
                      <td className="py-1.5 px-3 text-gray-400 w-12">{item.unit}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <h2 className="text-base font-semibold text-gray-800 mb-3">Building Requirements</h2>
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2.5">
          {Object.entries(FIELD_LABELS).map(([key, label]) => (
            <div key={key}>
              <dt className="text-xs text-gray-400 uppercase tracking-wide font-medium">{label}</dt>
              <dd className="text-sm text-gray-700 font-medium capitalize mt-0.5">
        ...
        {requirements.style && (
          <p className="mt-3 text-xs text-gray-500"><span className="font-medium">Style:</span> {requirements.style}</p>
        )}
        {qto && (
          <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-2 gap-3">
            <div className="bg-gray-50 rounded-lg p-2 text-center">
              <div className="text-lg font-bold text-gray-800">{qto.total_floor_area} m²</div>
              <div className="text-xs text-gray-500">Ground Floor Area</div>
            </div>
            <div className="bg-gray-50 rounded-lg p-2 text-center">
              <div className="text-lg font-bold text-gray-800">{qto.total_gross_area} m²</div>
              <div className="text-xs text-gray-500">Avg Floor Area</div>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Room Schedule */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <h2 className="text-base font-semibold text-gray-800 mb-3">Room Schedule (GF)</h2>
        <div className="space-y-1.5">
          {Object.entries(uniqueRooms).map(([name, count]) => (
            <div key={name} className="flex justify-between items-center text-sm">
              <span className="text-gray-600">{name}</span>
              <span className="text-gray-400 font-medium text-xs bg-gray-100 px-2 py-0.5 rounded">×{count}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {qto && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <h2 className="text-base font-semibold text-gray-800">Quantity Takeoff</h2>
      ...
      {/* Export */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <h2 className="text-base font-semibold text-gray-800 mb-3">Export</h2>
